...
func (s *AuthService) user_from_local(
	...
	auth_row := db.QueryRow(context.Background(), "SELECT tokens FROM user_auth_ WHERE _id=$1", user_auth.UserId)
	...
	var user User
	user_row := db.QueryRow(context.Background(), "SELECT _id, email, name, role FROM user_ WHERE _id=$1", user_auth.UserId)
...
func (s *AuthService) authenticate_and_get(
	...
	auth_row := db.QueryRow(context.Background(), "SELECT auth FROM user_auth_ WHERE _id=$1", user.Id.String())
	...
	if remember {
		remember_user_token(user.Id, user_auth_file_path, s.logger, db)
	} else {
...
func remember_user_token(user uuid.UUID, auth_file string, logger *slog.Logger,
	db *pgxpool.Pool,
) error {
	...
	_, err := db.Exec(context.Background(), append_token_query, auth_token, user)
	if err != nil {
		logger.With("error", err).Error("could not insert user auth token")
		return err
	}
	...
	if err != nil {
		logger.With("error", err).Error("could not save user auth token")
		return err
	}

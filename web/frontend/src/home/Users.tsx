...
function UserList() {
    ...
    return (
        <ul className="grid grid-cols-[repeat(4,min-content)_auto] gap-x-4">
    ...
    if (editing === user.Id.toString()) {
        ...
        return (
            <div className="contents">
                ...
                <div className="col-3">{user.Role}</div>
                <div className="col-4 pr-4 invisible group-hover/user-row:visible">
                    <button
                        type="button"
...
function UserItemEditing({
    ...
    const updateUserMutation = useMutation({
        mutationFn: user_service.update_user,
        onSettled: () =>
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY_USER_LIST] }),
    });
    const deactivateUserMutation = useMutation({
        mutationFn: user_service.deactivate_user,
        onSettled: () =>
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY_USER_LIST] }),
    });
    ...
    async function deactivate_user(e: MouseEvent<HTMLButtonElement>) {
        ...
        const update = {
            Id: user.Id,
            AccountStatus:
                types.AccountStatusDeactivated as types.AccountStatus,
            Email: user.Email,
            Name: user.Name,
            Role: user.Role,
        };
    ...
    return (
        <div className="contents">
            ...
            >
                ...
                <div className="col-4 flex gap-2 items-center">
            ...
            <div className="col-4 text-red-600">
                <small>{error}</small>
            </div>
        </div>
    );

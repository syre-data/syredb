...
export default function () {
  ...
  function add_column_if_needed(id: number, e: ChangeEvent<HTMLInputElement>) {
  ...
  function on_label_change(id: number, e: ChangeEvent<HTMLInputElement>) {
    ...
    for (const col of cols) {
      ...
      if (other_label === label) {
        input.setAttribute(LABEL_DUPLICATE_ERROR_KEY, col.id.toString());
        ...
        const other_dup_id = other.getAttribute(LABEL_DUPLICATE_ERROR_KEY);
  ...
  async function create_data_schema(e: FormEvent<HTMLFormElement>) {
    ...
    if (storage === null) {
      const input = document.getElementById("storage")! as HTMLSelectElement;
    ...
    for (const { id } of cols) {
      ...
      if (!dtype) {
        ...
        const input = document.getElementById(dtype_key)! as HTMLSelectElement;
  ...
  return (
    <div>
      <div>
        <h2 className="px-4 text-lg font-bold pb-4">Create a data schema</h2>
      </div>
      <div>
        <form
          id="schema-form"
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <div>
              <label>
                ...
                >
                  <option
                    value={app.DataStorage.DATA_STORAGE_INTERNAL}
                    title="Store data as a table in the database"
                  >
                    Internal
                  </option>
                  <option
                    value={app.DataStorage.DATA_STORAGE_FILE}
            ...
            <div>
              ...
              <ol>
                {cols.map((col, index) => (
                  <li key={col.id} className="flex gap-2 pb-2">
                    <div>{index + 1}.</div>
                    <ColumnSchema
                      schema={col}
          ...
          <div className="flex gap-2 justify-center pt-4">
            ...
            <div>
              <button type="button" className="btn-submit" onMouseDown={cancel}>
                Cancel
              </button>
            </div>
          </div>
...
function ColumnSchema({ schema, onRemove, onChangeLabel }: ColumnSchemaProps) {
  ...
  function on_change_label(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (input && !input.validity.customError && input.value.trim().length > 0) {
      input.setCustomValidity("");
    }

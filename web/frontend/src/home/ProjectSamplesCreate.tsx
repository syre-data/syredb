  ...
  switch (action.type) {
    ...
    case "add_property":
      if (
        ...
        console.error(`property ${action.payload.property.key} already exists`);
        return;
      }
...
function project_sample_create_is_empty(
  ...
  const properties_empty = !sample.Properties || sample.Properties.length === 0;
  const data_empty = sample.Data.length === 0;
  const notes_empty = sample.Notes.length === 0;

  return (
    label_empty && tags_empty && properties_empty && data_empty && notes_empty
  );
}
...
function ProjectSamplesCreate({ project_id }: ProjectSamplesCreateProps) {
  ...
  const { data: project } = useSuspenseQuery({
    ...
    queryFn: () => app.ProjectService.GetProjectWithUserPermission(project_id),
  });
  ...
  async function create_samples(form: HTMLFormElement) {
        ...
        switch (property.type) {
          ...
          case app.PropertyType.PROPERTY_TYPE_UINT:
            ...
            if (value < 0) {
              ...
              input.setCustomValidity("invalid value, must be non-negative");
            }
            break;
          ...
          case app.PropertyType.PROPERTY_TYPE_QUANTITY:
            ...
            const magnitude_value = parseFloat(property.value.magnitude);
            value = {
    ...
    const sample_data_filtered = [
      ...sample_info
        .entries()
        .filter(([_, sample]) => !project_sample_create_is_empty(sample)),
    ];
  ...
  return (
    <div>
      <div className="pt-1 px-4">
        <h2 className="font-bold">Create samples for {project.Label}</h2>
      </div>
          ...
          case "property":
            ...
            switch (data_property_key) {
              case "key":
                ...
                break;
              case "type":
                datum_property.type = property.type_string_to_variant(
                  value.toString(),
                );
                break;
              case "value":
                if (key.endsWith("[magnitude]")) {
                  ...
                  datum_property.value.magnitude = value.toString().trim();
                } else if (key.endsWith("[unit]")) {
                  if (datum_property.value === undefined) {
                    datum_property.value = {};
                  }
                  datum_property.value.unit = value.toString().trim();
                } else {
                  datum_property.value = value.toString().trim();
                }
                break;
              default:
                throw new Error(
...
function ProjectSamplesFormHeader() {
  ...
  function on_change_property_key(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (state.properties.findIndex((property) => property.key === key) > -1) {
  ...
  return (
    ...
    >
      ...
      <li className="px-4 col-1 -row-2 pl-1">
        <div>
          <button type="button" className="btn-cmd" onMouseDown={add_sample}>
            <icon.Plus />
          </button>
        </div>
      </li>
    ...
    <div className="group/sample-row contents">
      ...
      >
        <div className="flex gap-2 pb-2">
          <h3>Data</h3>
          <div>
            <button type="button" className="btn-cmd" onMouseDown={add_data}>
              <icon.Plus />
            </button>
          </div>
        </div>
      ...
      >
        <div className="flex gap-2 pb-2">
          <h3>Notes</h3>
          <div>
            <button type="button" className="btn-cmd" onMouseDown={add_note}>
              <icon.Plus />
            </button>
          </div>
        </div>
...
function SampleData({
    ...
    await app.DataService.ParseDataFileToSchema(file, schema)
      ...
      .catch((err) => {
        ...
        switch (err.message) {
          ...
          default:
            file_input.setCustomValidity("data incompatible with schema");
        }
  ...
  function add_property(key: string, type: app.PropertyType) {
    const id =
      properties.length === 0 ? 0 : properties[properties.length - 1].id + 1;
  ...
  return (
    <div className="col-span-full grid grid-cols-subgrid group/sample-data-property">
      <div className="col-1 whitespace-nowrap">
        ...
        {property.key} <span className="text-gray-500">({property.type})</span>
      </div>

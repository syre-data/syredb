...
func (s *ProjectService) CreateProjectSamples(project uuid.UUID, samples []ProjectSampleCreate) (Ok, error) {
	...
	err = s.create_project_samples_create_user_permissions(tx, sample_ids, user_id)
	if err != nil {
		return Ok{}, err
	}
...
func (s *ProjectService) create_project_samples_create_user_permissions(
	tx pgx.Tx,
	sample_ids []uuid.UUID,
	user_id uuid.UUID,
) error {
	...
	args[0] = user_id
	args[1] = SAMPLE_USER_PERMISSION_OWNER

	var query strings.Builder
	query.WriteString("INSERT INTO sample_user_permission_ (_sample, _user, _permission) VALUES ")
	for idx, sample_id := range sample_ids {
		if idx > 0 {
			query.WriteString(", ")
		}
...
type ProjectSampleResources struct {
	...
	DataSchemas       []DataSchema
	Users             []User
	UserPermissions   []SampleUserPermissions
}
...
func (s *ProjectService) GetProjectSampleResources(project_id uuid.UUID, sample_id uuid.UUID) (ProjectSampleResources, error) {
	...
	resources.ProjectMembership, err = s.get_project_sample_resources_project_membership(project_id, resources.Id)
	if err != nil {
		return ProjectSampleResources{}, nil
	}
	...
	resources.UserPermissions, err = s.get_project_sample_resources_user_permissions(sample_id, user_ids)
	if err != nil {
		return ProjectSampleResources{}, err
	}
...
func (s *ProjectService) get_project_sample_resources_project_membership(
	...
	membership.Creator, err = s.user_service.GetUserById(s.app_state.user_id)
	if err != nil {
		return ProjectSampleMembership{}, err
	}
...
func (s *ProjectService) get_project_sample_resources_sample_tags(
	project_id uuid.UUID,
	sample_id uuid.UUID,
) ([]string, error) {
...
func (s *ProjectService) get_project_sample_resources_user_permissions(sample_id uuid.UUID, user_ids []uuid.UUID) ([]SampleUserPermissions, error) {
	permission_query :=
		`SELECT _user, _permission FROM sample_user_permission_ 
		WHERE _sample=$1 AND _user=ANY($2)`
	rows, _ := s.db.conn.Query(s.ctx, permission_query, sample_id, user_ids)
	permissions := make([]SampleUserPermissions, len(user_ids))
	for idx, user_id := range user_ids {
		permissions[idx].User = user_id
	}
	...
	for rows.Next() {
		...
		if idx < 0 {
			s.logger.With("expected", user_ids, "found", user).Error("invalid user id")
